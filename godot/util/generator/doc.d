module godot.util.generator.doc;

import godot.util.generator.classes, godot.util.generator.methods, godot.util.generator.util;
    ...
    if (cDoc.hasChild("methods")) {
        ...
        foreach (mDoc; methods.children) {
            ...
            if (index != -1) {
                ...
                writefln("No method %s.%s", c.name.godotType, mDoc.attribute("name"));
        }
    }
    ...
    if (cDoc.hasChild("members")) {
        ...
        foreach (mDoc; members.children) {
            ...
            if (index != -1) {
                ...
                writefln("No property %s.%s", c.name.godotType, mDoc.attribute("name"));
        }
    }
...
void parseMethodDoc(GodotMethod m, DOMEntity!string mDoc) {
    if (mDoc.hasChild("description")) {
        ...
        if (fd.childText)
            m.ddoc = fd.childText.godotToDdoc;
    }
}
