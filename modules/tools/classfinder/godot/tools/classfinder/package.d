...
+/
module godot.tools.classfinder;
...
size_t startLocation(in BaseNode node) {
    return node.tokens.front.index;
}

size_t endLocation(in BaseNode node) {
    return node.tokens.back.index;
}
...
private class GDVisitor : ASTVisitor {
    ...
    version (none) override void visit(in AtAttribute a) {
        ...
        if (a.argumentList.items.canFind!(e => (cast(PrimaryExpression) e) && (
                cast(PrimaryExpression) e).primary.text == "MainClass")) {
    ...
    override void visit(in MixinTemplateName m) {
        ...
        if (m.tokens.canFind!(t => t.text == "GodotNativeLibrary"))
            file.hasEntryPoint = true;
    }

    override void visit(in StructDeclaration s) {
        ScopeRange range = ScopeRange(s.name.text, ScopeRange.Type.struct_, s.startLocation, s
                .endLocation);
        scopeRanges ~= range;
    }

    override void visit(in InterfaceDeclaration i) {
        ScopeRange range = ScopeRange(i.name.text, ScopeRange.Type.struct_, i.startLocation, i
                .endLocation);
        scopeRanges ~= range;
    }
    ...
    override void visit(in ClassDeclaration c) {
        ...
        if (c.name.text.toLower == moduleName.back || c.name.text.camelToSnake == moduleName.back) {
            if (!file.mainClass.empty)
                writefln!"Module %s: found multiple classes matching the module name (%s and %s)"(
...
FileInfo parse(string path) {
    ...
    if (visitor.file.mainClass.empty && visitor.file.classes.length == 1)
        visitor.file.mainClass = visitor.file.classes[0];
    ...
    foreach (ci, c; visitor.file.classes) {
        ...
        visitor.file.classes[ci] = chain(only(visitor.file.moduleName), parentScopeNames).joiner(".")
            .text;
    }
