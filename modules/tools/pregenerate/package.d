// module godot.tools.pregenerate;
import godot.tools.classfinder;
...
enum entryPointRoot = "/// Godot-DLang entry point";
static immutable string entryPointSource = q{$ENTRYPOINTROOT
/// This file was automatically generated by godot-dlang:pregenerate.
...
string[] sanitized(const string paths) {
    ...
    if (!parts.front.isRooted)
        assert(0, "Paths passed to sanitized() are not absolute paths!");
        ...
        if (exists(importPath) && isDir(importPath)) {
            foreach (DirEntry de; dirEntries(importPath, SpanMode.depth)) {
                if (de.isFile && de.name.endsWith(".d")) {
                    if (!sourceFiles.canFind(de.name))
                        sourceFiles ~= de.name;
                }
            }
        }
	...
	************************************************************************ */
    ...
    if (viewsEnv.empty) {
        ...
        if (classesFile.empty) {
            ...
            if (found.length)
                viewsDir = found.front;
            else
                viewsDir = splitted.front;
            classesFile = viewsDir.buildPath(classesFilename);
        }
    }
	...
	************************************************************************ */
    ...
    if (makeEntryPoint) {
        ...
        if (found.length) {
            ...
            if (found.front.name.endsWith("entrypoint.d"))
                entryPointFile = found.front.name;
        } else {
            entryPointFile = importPaths.front.buildPath(entryPointFilename);
            if (entryPointFile.exists)
                assert(0, entryPointFile ~ " exists but does not contain the GodotNativeLibrary mixin.");
        }
    }
