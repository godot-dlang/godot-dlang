...
mixin template GodotNativeLibrary(string symbolPrefix, Args...)
{
		...
		GDNativeExtensionClassLibraryPtr p_library, GDNativeInitialization* r_initialization)
	{
		...
		import godot.d.traits;
		static import godot.d.register;
		version(Windows) {}
		else
		{
		...
		godot.d.register._GODOT_library = p_library;
	...
	{
		import std.meta, std.traits;
		import godot.d.register : register, fileClassesAsLazyImports;
...
void register(T)(GDNativeExtensionClassLibraryPtr lib) if(is(T == class))
{
	...
	else // base class is another D script
	{
		...
		static if(hasUDA!(Base, Rename)) enum immutable(char*) baseName = godotName!Base ~ '\0';
		else enum immutable(char*) baseName = __traits(identifier, Base) ~ '\0';
	}

	static if(hasUDA!(T, Rename)) enum immutable(char*) name = godotName!T ~ '\0';
	else enum immutable(char*) name = __traits(identifier, T) ~ '\0';
	...
	extern(C) static GDNativeExtensionClassCallVirtual getVirtualFn(void* p_userdata, const char *p_name) 
	{
		...
		if (strcmp(p_name, "_ready") == 0)
		{
			...
			return cast(GDNativeExtensionClassCallVirtual) &OnReadyWrapper!(T, __traits(getMember, T, "_ready")).callOnReady;
		}
	...
	{{	
		static if(hasUDA!(mf, Rename)) enum string externalName = godotName!mf;
		else enum string externalName = (fullyQualifiedName!mf).replace(".", "_");
		registerMethod!mf();
	}}
	...
	{{
		...
		static if(hasUDA!(s, Rename)) enum string externalName = godotName!s;
		else enum string externalName = (fullyQualifiedName!s).replace(".", "_");
		...
		_godot_api.classdb_register_extension_class_signal(lib, name.ptr, (externalName).ptr, prop.ptr, Parameters!s.length);
	}}
	...
	{{
		...
		_godot_api.classdb_register_extension_class_property(lib, name.ptr, &pinfo, set_prop.ptr, get_prop.ptr);
	}}
	...
	{{
		...
		_godot_api.classdb_register_extension_class_integer_constant(lib, name.ptr, null, pName, cast(int) E, false);
	}}
